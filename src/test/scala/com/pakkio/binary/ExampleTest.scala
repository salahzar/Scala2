package com.pakkio.binary

import Implicits._

import org.scalatest.{fixture, FunSuite}

object Unzipper {

  implicit class Unzip4[A, B, C, D](val xs: List[(A, B, C, D)]) extends AnyVal {
    def unzip4: (List[A], List[B], List[C], List[D]) = xs.foldRight[(List[A], List[B], List[C], List[D])]((Nil, Nil, Nil, Nil)) { (x, res) =>
      val (a, b, c, d) = x
      (a :: res._1, b :: res._2, c :: res._3, d :: res._4)
    }
  }

}

import Unzipper._

/**
 * Created by Francesco on 09/08/2015.
 */
class ExampleTest extends FunSuite {
  val msgs = List(

    ""
    , "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
    , "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
    , ""
    , "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
    , ""
    , ""
    , "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
    , ""
    , "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
    , "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
  )
  val key = "aaaa"


  val origM = List(
    "the first thing to think when we are ready to go out is to be happy"
    , "somebody can tell you that we are not good to do the good, but we"
    , "are sure we can do that this is the third and last message do try"
    , " to decrypt a story in the world is where you can learn from your"
    , " errors similarities are often casuals and cannot be reproduced s"
    , "ome people say the same as other people, but we cannot guarantee."
  )

  def decrypt(C: List[Buffer]): Unit = {

    // gets some statistics on messages
    val maxLengthFound = C.map(_.content.length).max
    val num = C.size

    // initial Key is empty filled with None
    val EMPTY: List[Option[Byte]] = List.fill(maxLengthFound)(None)
    var K = EMPTY
    // generates num messages each of them with a string of unknown characters
    var M = Array.fill(num)(EMPTY)

    val combinations = (0 until num).combinations(3).toList
    combinations foreach handle_message

    M.zip(C).
      map { pair => {
        val b=Buffer.fromOB(pair._1).toString()
        println(b.substring(0, pair._2.content.length)
      )
    }
    }

    case class GivenData(k: Option[Byte],
                         m0: Option[Byte], m0Len: Int,
                         m1: Option[Byte], m1Len: Int,
                         m2: Option[Byte], m2Len: Int,
                         c0: Byte, c1: Byte, c2: Byte)
    case class ReturnedData(m0: Option[Byte], m1: Option[Byte], m2: Option[Byte], k: Option[Byte])

    // handle a triplet giving their indices with all their permutations
    def handle_message(l: Seq[Int]) = {
      //list.permutations.map(OperateOnThisTriplet)
      OperateOnThisTriplet(List(l(0), l(1), l(2)))
      OperateOnThisTriplet(List(l(1), l(2), l(0)))
      OperateOnThisTriplet(List(l(2), l(0), l(1)))

    }
    def OperateOnThisTriplet(list: Seq[Int]) = {
      val m = list.map(M(_))
      val Seq(i0, i1, i2) = list
      val (newm0, newm1, newm2, newkey) = m(0).zipWithIndex.map { pair =>
        val (msg_char, column) = pair
        val r = handleByteIndex(column, i0, i1, i2)
        (r.m0, r.m1, r.m2, r.k)

      }.toList.unzip4

      M(i0) = newm0
      M(i1) = newm1
      M(i2) = newm2
      K = newkey
    }
    def handleByteIndex(column: Int, i0: Int, i1: Int, i2: Int)
    : ReturnedData = {

      def handleByte(g: GivenData)
      : ReturnedData = {

        def retKnowingKey(k: Byte) = {

          ReturnedData(
            Some((g.c0 ^ k).toByte),
            Some((g.c1 ^ k).toByte),
            Some((g.c2 ^ k).toByte),
            Some(k)
          )
        }
        if (g.k.isDefined) retKnowingKey(g.k.get)
        else if (g.m0.isDefined) retKnowingKey((g.m0.get ^ g.c0).toByte)
        else {
          val xor01 = (g.c0 ^ g.c1).toByte
          val xor02 = (g.c0 ^ g.c2).toByte
          val xor12 = (g.c1 ^ g.c2).toByte
          if ((ifspace(xor01) && ifspace(xor02)) && (xor12 != 0) && (column < g.m0Len))
          // we have sure space in m1 so we can compute the key
            retKnowingKey((32 ^ g.c0).toByte)

          else
            ReturnedData(g.m0, g.m1, g.m2, g.k)
        }
      }


      val given_data = GivenData(
        K(column),
        M(i0)(column), C(i0).content.length,
        M(i1)(column), C(i1).content.length,
        M(i2)(column), C(i2).content.length,
        if (column < C(i0).content.length) C(i0).content(column) else 0,
        if (column < C(i1).content.length) C(i1).content(column) else 0,
        if (column < C(i2).content.length) C(i2).content(column) else 0
      )

      handleByte(given_data)

    }

  }

  test("crypting three messages with same KEY") {

    val maxLengthFound = origM.map(_.length).max

    // encrypts all the messages being sure to know exactly which key we are using
    val origK = CryptoExtend(key).extend(maxLengthFound)
    val C = origM.map(_.crypt(key))

    decrypt(C)


  }
  test("various messages") {

    val C = msgs.map(Buffer.fromHex(_))
    decrypt(C)

  }
}
